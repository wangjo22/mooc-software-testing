...
/*
Thinking about boundaries and corner cases is not an easy task. However, developers often do common mistakes, which may serve as hints for us when testing any software system.

In this unit, we'll tell you about the CORRECT way. You can read more about them in the "Pragmatic Unit Testing in Java 8 with JUnit" book, from Jeff Langr, with Andy Hunt & Dave Thomas, edited by the Pragmatic Programmers, edition of 2015.

The CORRECT way stands for Conformance, Ordering, Range, Reference, Existence, Cardinality, and Time:

Conformance: Many inputs should conform to a specific format. For example, an e-mail address has a name@domain format. What happens to our system if the e-mail is invalid, e.g, no name, or no domain, or invalid domain?
...
Reference: The inputs and behaviour under test may require a previous state. For example, in a web store, for an order to be placed, the user needs to have an account first. What happens if the user tried to place an order without an account?
Existence: The inputs "may not exist". What happens if someone passes a null or an empty string?
Cardinality: The famous off-by-one error. It happens when we forget to "iterate the last item" in our algorithm. For example, if you should return the number of elements between m and n (inclusive), the intuitive answer would be (n-m), whereas the right answer is (n-m+1). In practice, this often happens in iterative loops: developers often iterate one time too few. You can read about the fencepost error in the Wikipedia. As a tip, what happens to your system when it has zero, one, or many values to iterate?
